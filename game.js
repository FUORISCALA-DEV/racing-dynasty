...
const I18N = {
  it: {
    ...
    dh_quit_confirm: 'Vuoi davvero uscire? La stagione in corso andrà persa (il salvataggio automatico arriva al punto 4).',
    dse_title: 'Stagione conclusa', dse_subtitle: 'Verifica punto 3 completata — il punto 4 (prestigio/età/contratti) gestirà questo momento per davvero.',
 ...
 friends_badge_label: 'AMICI', premium_badge_tooltip: 'Membro Immortale', friends_my_code_title: 'Il tuo codice invito', friends_my_code_desc: 'Condividilo con un amico (WhatsApp, Discord...) — lo inserisce lui per aggiungerti.',
  ...
  en: {
    ...
    dh_quit_confirm: "Do you really want to quit? The current season will be lost (autosave arrives at step 4).",
    dse_title: 'Season over', dse_subtitle: 'Step 3 verification complete — step 4 (prestige/age/contracts) will handle this moment for real.',
 ...
 friends_badge_label: 'FRIEND', premium_badge_tooltip: 'Immortal Member', friends_my_code_title: 'Your invite code', friends_my_code_desc: 'Share it with a friend (WhatsApp, Discord...) — they enter it to add you.',
  ...
  es: {
    ...
    dh_quit_confirm: '¿Seguro que quieres salir? La temporada en curso se perderá (el autoguardado llega en el paso 4).',
    dse_title: 'Temporada terminada', dse_subtitle: 'Verificación del paso 3 completa — el paso 4 (prestigio/edad/contratos) gestionará este momento de verdad.',
...
// V0.9.7.9.23: vibrazione a vero ritmo cardiaco (lub-dub, pausa, lub-dub...) per il ballottaggio
// delle decisioni in gara — intense=true per le "tutto o niente" ad alto rischio, pattern piu'
// stretto e piu' lungo, si sente davvero la differenza rispetto a una decisione normale.
function triggerHeartbeatHaptic(intense, durationMs){
  if(audioSettings.hapticEnabled===false || !navigator.vibrate) return;
  const beat = intense ? [40,90,40,220] : [25,70,25,380];
  const cycleLen = beat.reduce((a,b)=>a+b,0);
  const reps = Math.max(1, Math.round(durationMs/cycleLen));
  let full = [];
  for(let i=0;i<reps;i++) full = full.concat(beat);
  try{ navigator.vibrate(full); }catch(e){ /* ignorato */ }
}
...
// ============================================================
// V0.9.7.9.5 — CARRIERA PILOTA (punto 3/8): Hub + stagione giocabile
// Riusa quasi verbatim il motore di simulazione di Carriera Scuderia (simulateFullRace,
// renderPregara, renderRaceLiveInit, renderRaceResult) costruendo un oggetto 'state' compatibile
// — stessa forma (team/grid/aiTeams/calendar/driverStandings), dati diversi. Le due modalita'
// restano completamente separate nei salvataggi, ma condividono lo stesso motore di gara a runtime.
const DRIVER_CAREER_TIER_DRAWS = { kart:1, minore:2, elite:4 };

// Assegna il giocatore a UNA scuderia scelta a caso nella Serie indicata, genera il compagno di
// scuderia e i 5 componenti condivisi, scalati per forza in base al livello (stesso meccanismo
// "pesca N, scegli il migliore" gia' usato per le scuderie IA di Carriera Scuderia).
function assignDriverCareerTeam(tierName, usedIds){
  const teamIds = Object.keys(driverCareerState.world.tiers).filter(id=> driverCareerState.world.tiers[id]===tierName);
  const teamId = teamIds[Math.floor(rnd()*teamIds.length)];
  const teamData = DATA.scuderie.find(s=>s.id===teamId);
  ...
  return { teamId, teamData, teammate, components };
}

// Griglia di 20 slot: le 10 scuderie della Serie del giocatore, 2 piloti ciascuna — stessa forma
// dati di state.grid in Carriera Scuderia (buildAIGrid), ma scoped alla Serie invece che a un
// campione casuale di 9 scuderie su 30.
function buildDriverCareerGrid(tierName, playerAssignment, usedIds){
  const teamIds = Object.keys(driverCareerState.world.tiers).filter(id=> driverCareerState.world.tiers[id]===tierName);
  const n = DRIVER_CAREER_TIER_DRAWS[tierName] || 2;
  const aiTeams = teamIds.filter(id=>id!==playerAssignment.teamId).map(id=>{
    const teamData = DATA.scuderie.find(s=>s.id===id);
    const d1 = pickBestOfNDistinct(DATA.piloti.filter(p=>p.rating<100), n, usedIds); usedIds.add(d1.id);
    const d2 = pickBestOfNDistinct(DATA.piloti.filter(p=>p.rating<100), n, usedIds); usedIds.add(d2.id);
    const components = {
      motore: pickBestOfNDistinct(DATA.motori.filter(x=>x.rating<100), n, usedIds),
      telaio: pickBestOfNDistinct(DATA.telai.filter(x=>x.rating<100), n, usedIds),
      aero: pickBestOfNDistinct(DATA.aero.filter(x=>x.rating<100), n, usedIds),
    ...
    const t = { ...teamData, id, drivers:[d1,d2], components, tier: tierName };
    applyAITeamSynergyBonuses(t);
    return t;
  });
  ...
  const grid = [];
  let carNum = 1;
  grid.push({ slotKey:'PLAYER-1', teamId:'PLAYER', teamName:playerAssignment.teamData.nome, isPlayerTeam:true, role:'pilotMain', carNumber:carNum++ });
  grid.push({ slotKey:'PLAYER-2', teamId:'PLAYER', teamName:playerAssignment.teamData.nome, isPlayerTeam:true, role:'pilotSecond', carNumber:carNum++ });
  aiTeams.forEach(t=>{
    grid.push({ slotKey:t.id+'-1', teamId:t.id, teamName:t.nome, isPlayerTeam:false, role:0, carNumber:carNum++ });
    grid.push({ slotKey:t.id+'-2', teamId:t.id, teamName:t.nome, isPlayerTeam:false, role:1, carNumber:carNum++ });
  });
  return { grid, aiTeams };
}

// Costruisce lo state compatibile con Carriera Scuderia per una stagione di Carriera Pilota.
// Stagione fissa a 10 gare (formato Veloce) — piu' semplice per iniziare, si puo' rivedere.
const DRIVER_CAREER_SEASON_LENGTH = 10;
function startDriverCareerSeason(){
  const usedIds = new Set();
  const tierName = driverCareerState.currentTier || 'kart';
  const playerAssignment = assignDriverCareerTeam(tierName, usedIds);
  ...
  driverCareerState.currentTeamId = playerAssignment.teamId;

  state = {
    phase: 'driver-hub',
    isDriverCareer: true, // V0.9.7.9.5: distingue questo state da una normale Carriera Scuderia
    difficulty: 'medio', // usato solo per DIFFICULTY_LABEL nella topbar, non incide sui rerolls (qui non ce ne sono)
    seasonLength: DRIVER_CAREER_SEASON_LENGTH,
    raceIndex: 0,
    calendar: shuffledCircuits,
    aiTeams,
    team: {
    ...
    grid,
    driverStandings: {}, exCounter: 0, constructorStandings: {},
...
function renderDriverHub(){
  ...
  <div class="btnrow"><button class="ghost" data-action="quit-driver-career">${t('dh_quit')}</button></div>
  `;
  bindActions();
}
...
const DECISION_OUTCOME_BUCKETS = {
  ...
  try_overtake_desperate: [{ prob:0.45, label:'gain_3_5', min:-6, max:-4 }, { prob:0.55, label:'lose_3', min:3, max:3 }],
  give_up_attack:         [{ prob:1.00, label:'hold', min:0, max:0 }],
  lighten_fuel:           [{ prob:0.55, label:'gain_1_2', min:-4, max:-2 }, { prob:0.45, label:'lose_3', min:3, max:3 }],
  ...
  push_limit:             [{ prob:0.60, label:'gain_1_2', min:-2, max:-1 }, { prob:0.40, label:'lose_2', min:2, max:2 }],
  ...
  wait_for_space:         [{ prob:0.15, label:'gain_1', min:-1, max:-1 }, { prob:0.85, label:'hold', min:0, max:0 }],
  push_brakes:            [{ prob:0.58, label:'gain_1_2', min:-2, max:-1 }, { prob:0.42, label:'lose_2', min:2, max:2 }],
...
function liveDecisionRevealHTML(){
  ...
  const highStakes = !!reveal.highStakes;
  const slotNames = {
...
// non gestisce componenti). Fine stagione (punto 4: prestigio/invecchiamento/contratti/promozioni)
...
function renderDailySeasonHub(){
  ...
  // V0.9.9.140: contatore gettoni Daily (x/3 premium, x/1 free) e pulsante "Nuova run" — richiesti
  // da Gio. Il pulsante "Nuova run" abbandona il salvataggio locale in corso e ne avvia uno nuovo
...
function renderModeSelect(){
  ...
  <div class="grid grid-2 mode-select-grid">
    ...
    <div class="card pickable mode-select-card ${isDailySeasonUnlocked()?'':'mode-select-card-locked'}" data-rarity="Rare" data-action="${isDailySeasonUnlocked()?'open-daily-season-hub':'daily-locked-info'}">
      <img class="mode-select-bg-img ${isDailySeasonUnlocked()?'':'mode-select-bg-img-locked'}" src="assets/mode-select/daily-season.webp" alt="">
      <div class="mode-select-top">
...
const ACHIEVEMENTS = [
  ...
  { id:'senza-rete-di-sicurezza', cat:'Difficile', title:'Senza Rete di Sicurezza', desc:'Vinci una stagione a difficoltà Elite.', en:{title:'No Safety Net', desc:'Win a season on Elite difficulty.'}, es:{title:'Sin Red de Seguridad', desc:'Gana una temporada en dificultad Elite.'} },
  ...
  { id:'osso-duro', cat:'Difficile', title:'Osso Duro', desc:'Completa una stagione Elite senza sostituire mai nulla.', en:{title:'Tough Nut', desc:'Complete an Elite season without ever replacing anything.'}, es:{title:'Hueso Duro de Roer', desc:'Completa una temporada Elite sin sustituir nunca nada.'} },
  { id:'il-migliore-che-ci-sia', cat:'Difficile', title:'Il Migliore Che Ci Sia', desc:'Grande Slam a difficoltà Elite.', en:{title:'The Best There Is', desc:'Grand Slam on Elite difficulty.'}, es:{title:'El Mejor Que Existe', desc:'Grande Slam en dificultad Elite.'} },
  ...
  { id:'costanza-chirurgica', cat:'Estremo', title:'Costanza Chirurgica', desc:'Vinci una stagione Elite completa restando sempre sul podio.', en:{title:'Surgical Consistency', desc:'Win a full Elite season always finishing on the podium.'}, es:{title:'Constancia Quirúrgica', desc:'Gana una temporada Elite completa quedando siempre en el podio.'} },
  { id:'l-impossibile-fatto-possibile', cat:'Estremo', title:'L\'Impossibile Fatto Possibile', desc:'Grande Slam Elite, stagione completa, partendo da rating scuderia sotto 60.', en:{title:'The Impossible Made Possible', desc:'Elite Grand Slam, full season, starting from a team rating below 60.'}, es:{title:'Lo Imposible Hecho Posible', desc:'Grande Slam Elite, temporada completa, partiendo de un rating de escudería inferior a 60.'} },
...
function checkSeasonEndAchievements(){
  ...
  // V0.9.9.96: DAILY SEASON — tracciamento separato dei due titoli, indipendenti tra loro (non serve
  // più vincerli nella stessa stagione, come chiarito da Gio: "posso farle anche tutte e due insieme"
  // implicava che farle insieme fosse un modo IN PIÙ, non l'unico). Usato sia per lo sblocco della
  // Daily Season sia per i due indicatori rapidi mostrati sulla carta.
  if(!state.isTutorialRun && !state.isDailySeason){
    if(isConstructorChamp && !achievementData.everWonConstructorTitle){
      achievementData.everWonConstructorTitle = true; saveAchievementData();
    }
    if(isDriverChamp && !achievementData.everWonDriverTitle){
      achievementData.everWonDriverTitle = true; saveAchievementData();
    }
  }
...
function dailySeasonLockedMessageHTML(){
  ...
  const parti = [];
  if(missingTrophies>0) parti.push(t('daily_locked_trophies', missingTrophies));
  if(missingConstructor || missingDriver) parti.push(t('daily_locked_titles_v2'));
  return parti.map(p=>`<div style="margin-top:8px;">${p}</div>`).join('');
}
...
function rivalPanelHTML(){
  ...
  const rows = state.rivals.map(id=>{
    ...
    const logoBox = logoMatch
      ? `<div class="rival-trio-cell rival-trio-logo-cell"><img class="rival-trio-logo" src="assets/team-logos/${logoMatch.id}.webp" alt=""></div>`
      : `<div class="rival-trio-cell rival-trio-logo-cell"></div>`;
    return `<div class="rival-card-full">
      ...
      <div class="rival-trio-row">
        ${logoBox}
        <div class="rival-trio-cell rival-mini">
          ${carVisualHTML(t.drivers[0], t.components, slot1?slot1.carNumber:1, undefined, undefined, t.nome)}
          <div class="rival-mini-name">${flag(t.drivers[0].naz)} ${t.drivers[0].nome}</div>
          <div class="dim mono" style="font-size:11px;">${t.drivers[0].rating} RATING</div>
        </div>
...
function onAction(e){
  ...
  else if(action==='daily-new-run-instead'){
    gameConfirm(t('daily_new_run_confirm_desc'), ()=>{
      deleteDailyGameSave();
      enterDailySeasonFlow();
    }, t('daily_new_run_confirm_title'));
  }
...
// V0.9.9.101: prima di andare dritti su Stripe, mostriamo una scelta — richiesto da Gio: chi ha
// un codice gratuito non deve rischiare di pagare per sbaglio cliccando "Diventa Immortale".
function openPremiumChoicePanel(){
  document.getElementById('premiumChoiceMessage').textContent = t('premium_choice_message');
  document.getElementById('premiumChoiceCodeBtn').textContent = t('premium_choice_code_btn');
  document.getElementById('premiumChoicePayBtn').textContent = t('premium_choice_pay_btn');
  document.getElementById('premiumChoiceCancelBtn').textContent = t('premium_choice_cancel_btn');
  const panel = document.getElementById('premiumChoicePanel');
  const codeBtn = document.getElementById('premiumChoiceCodeBtn');
  const payBtn = document.getElementById('premiumChoicePayBtn');
  const cancelBtn = document.getElementById('premiumChoiceCancelBtn');
  function cleanup(){
  ...
  function onCode(){ cleanup(); openRedeemCodePanel(); }
  function onPay(){ cleanup(); startPremiumCheckout(); }
...
function guidePanelHTML(){
  ...
  const sectionsEn = [
    ...
    { title:'Difficulty and Season Length', body:`
      ...
      <p><b>Difficulty</b> (Easy → Elite) mainly affects the rerolls available at the Draft — the higher it is, the less choice you have, and the more on-the-spot decisions matter.</p>` },
    { title:'Trophy Room, Dynasty Museum and Achievements', body:`
      <p>The <b>Trophy Room</b> shows a trophy for every circuit in the game: gold if you've already won it, dim if not yet — it persists across all your careers. The <b>Dynasty Museum</b> collects every driver and component you've ever owned, in any career. <b>Achievements</b> are special milestones (perfect seasons, Elite challenges, particular playstyles) that unlock automatically as you play: you'll find them in the dedicated menu entry.</p>` },
  ];
  const sectionsEs = [
